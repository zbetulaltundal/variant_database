...
def handle_other_infos(conn, info, info_id):
    ...
    try:
        #'CLNDISDB'
        if main.search_field(info, 'CLNDISDB') != None:
...
def import_clinvar_data(conn):
    ...
    for rec in vcf_reader:
        ...
        query = """INSERT INTO variant (CHROM, POS, VAR_ID, \
...
def handle_other_infosv3(conn, info, info_id):
                    ...
                    for sub_str in val_str.split('|'):
                        ...
                        if main.check_var(values) != None:
                            if len(values) == 2:
                                ...
                                #if exists get the id of that rec
...
def import_clinvar_datav3(conn):
